...
sensor_msgs::PointCloud PC2Mapframe( 
	const float pose_x, const float pose_y, const float pose_th,
	const sensor_msgs::PointCloud& pc){
	...
	// transforming points
	float c = cos(pose_th), s = sin(pose_th);
	for (int i=0; i < pc.points.size(); i++){
		pctf.points.at(i).x = c*pc.points.at(i).x - s*pc.points.at(i).y + pose_x;
		pctf.points.at(i).y = s*pc.points.at(i).x + c*pc.points.at(i).y + pose_y;
		pctf.points.at(i).z =   pc.points.at(i).z;
	};
...
sensor_msgs::PointCloud Map2PCframe( 
	const float pose_x, const float pose_y, const float pose_th,
	const sensor_msgs::PointCloud& pc){
	...
	float c = cos(pose_th), s = sin(pose_th);
	for (int i=0; i < pc.points.size(); i++){
		pctf.points.at(i).x = c*pc.points.at(i).x + s*pc.points.at(i).y - c*pose_x - s*pose_y;
		pctf.points.at(i).y =-s*pc.points.at(i).x + c*pc.points.at(i).y + s*pose_x - c*pose_y;
		pctf.points.at(i).z =   pc.points.at(i).z;
	};
...
std::tuple<sensor_msgs::PointCloud, std::vector<float>> extractPlants(
							 const sensor_msgs::PointCloud& pc )
{
	...
	stats.push_back(xbar); stats.push_back(CI);
	return std::make_tuple(pcex, stats);
}
...
Eigen::MatrixXf line_detection( const sensor_msgs::PointCloud& center ){
